...
#### Do I need a license to use Avalonia?[​](#do-i-need-a-license-to-use-avalonia "Direct link to Do I need a license to use Avalonia?")
...
#### What if I don't want any of this?[​](#what-if-i-dont-want-any-of-this "Direct link to What if I don't want any of this?")
...
#### Can I continue using the legacy tools?[​](#can-i-continue-using-the-legacy-tools "Direct link to Can I continue using the legacy tools?")
...
#### When does my Community License expire?[​](#when-does-my-community-license-expire "Direct link to When does my Community License expire?")
...
#### What happens after the Visual Studio grace period?[​](#what-happens-after-the-visual-studio-grace-period "Direct link to What happens after the Visual Studio grace period?")
...
#### I have another question. Where can I ask?[​](#i-have-another-question-where-can-i-ask "Direct link to I have another question. Where can I ask?")
...
#### Is it possible to connect multiple instances to the Developer Tools?[​](#is-it-possible-to-connect-multiple-instances-to-the-developer-tools "Direct link to Is it possible to connect multiple instances to the Developer Tools?")

Yes, while a single instance of Developer Tools is running and activated, you can connect one or many apps to it. Each new connection will open a new Developer Tools window, working independently from each other.

#### Does it work with Browser/Android/iOS?[​](#does-it-work-with-browserandroidios "Direct link to Does it work with Browser/Android/iOS?")

It does work with mobile and browser applications. See [Attaching Browser or Mobile application](/tools/developer-tools/attaching-applications.md) for more details.

#### Can I use Developer Tools and DiagnosticsPackage with NativeAOT app?[​](#can-i-use-developer-tools-and-diagnosticspackage-with-nativeaot-app "Direct link to Can I use Developer Tools and DiagnosticsPackage with NativeAOT app?")

Yes. DiagnosticsPackage is fully trimming friendly. Even though it does use reflection, the tool was tested with AOT.

#### Does `AvaloniaUI.DiagnosticsSupport` replace `Avalonia.Diagnostics` package? Or do I need both?[​](#does-avaloniauidiagnosticssupport-replace-avaloniadiagnostics-package-or-do-i-need-both "Direct link to does-avaloniauidiagnosticssupport-replace-avaloniadiagnostics-package-or-do-i-need-both")

You only need `AvaloniaUI.DiagnosticsSupport`. `Avalonia.Diagnostics` is an old package used for legacy developer tools. It can be safely removed from the project. If for some reason necessary, both packages can be referenced, but you might want to setup different gestures for each tool.

#### Can everybody build project referencing `AvaloniaUI.DiagnosticsSupport`, even without a license?[​](#can-everybody-build-project-referencing-avaloniauidiagnosticssupport-even-without-a-license "Direct link to can-everybody-build-project-referencing-avaloniauidiagnosticssupport-even-without-a-license")
...
#### Is it necessary to exclude `AvaloniaUI.DiagnosticsSupport` package from Release/Production build?[​](#is-it-necessary-to-exclude-avaloniauidiagnosticssupport-package-from-releaseproduction-build "Direct link to is-it-necessary-to-exclude-avaloniauidiagnosticssupport-package-from-releaseproduction-build")
...
#### Are arm64 and x86 builds of the tool available or planned?[​](#are-arm64-and-x86-builds-of-the-tool-available-or-planned "Direct link to Are arm64 and x86 builds of the tool available or planned?")

Only **x64** builds for Windows and Linux are available at the moment. macOS build is an universal bundle with both **x64** and **arm64** architectures.

## TreeDataGrid[​](#treedatagrid "Direct link to TreeDataGrid")

### Data Updates[​](#data-updates "Direct link to Data Updates")

#### TreeDataGrid doesn't update when I change model properties[​](#treedatagrid-doesnt-update-when-i-change-model-properties "Direct link to TreeDataGrid doesn't update when I change model properties")
...
#### New items don't appear when I add them to the collection[​](#new-items-dont-appear-when-i-add-them-to-the-collection "Direct link to New items don't appear when I add them to the collection")
...
### Cell Editing[​](#cell-editing "Direct link to Cell Editing")

#### Cell editing doesn't work when I click on cells[​](#cell-editing-doesnt-work-when-i-click-on-cells "Direct link to Cell editing doesn't work when I click on cells")
...
## WebView[​](#webview "Direct link to WebView")

#### Is offscreen rendering supported? To avoid airspace issue?[​](#is-offscreen-rendering-supported-to-avoid-airspace-issue "Direct link to Is offscreen rendering supported? To avoid airspace issue?")

No, offscreen rendering is not currently supported. Offscreen rendering is being tracked as a potential future feature.

#### Why is NativeWebView control not supported on Linux?[​](#why-is-nativewebview-control-not-supported-on-linux "Direct link to Why is NativeWebView control not supported on Linux?")
...
#### Can I use WebAuthenticationBroker for Google Auth or Microsoft.Identity Auth?[​](#can-i-use-webauthenticationbroker-for-google-auth-or-microsoftidentity-auth "Direct link to Can I use WebAuthenticationBroker for Google Auth or Microsoft.Identity Auth?")
...
#### Why use WebAuthenticationBroker over other options?[​](#why-use-webauthenticationbroker-over-other-options "Direct link to Why use WebAuthenticationBroker over other options?")
...
#### Does NativeWebView support camera/microphone/screenshare access via getUserMedia() API?[​](#does-nativewebview-support-cameramicrophonescreenshare-access-via-getusermedia-api "Direct link to Does NativeWebView support camera/microphone/screenshare access via getUserMedia() API?")
...
* [Community edition](/tools/community-edition.md)
