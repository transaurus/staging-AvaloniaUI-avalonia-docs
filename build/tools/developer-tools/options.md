...
## DeveloperToolsOptions.Gesture[​](#developertoolsoptionsgesture "Direct link to DeveloperToolsOptions.Gesture")

Defines the gesture to run and connect to the `Developer Tools` process. By default: `F12`.

## DeveloperToolsOptions.ApplicationName[​](#developertoolsoptionsapplicationname "Direct link to DeveloperToolsOptions.ApplicationName")

Optional application display name. If unset, `Application.Name` or entry assembly name is used.

## DeveloperToolsOptions.ConnectOnStartup[​](#developertoolsoptionsconnectonstartup "Direct link to DeveloperToolsOptions.ConnectOnStartup")

Defines if the app should be connected to dev tools on startup. By default: `true` on iOS and Android, `false` everywhere else.

## DeveloperToolsOptions.AutoConnectFromDesignMode[​](#developertoolsoptionsautoconnectfromdesignmode "Direct link to DeveloperToolsOptions.AutoConnectFromDesignMode")

Defines if design mode app should be connected to dev tools. Default is 'false'.

## DeveloperToolsOptions.Runner[​](#developertoolsoptionsrunner "Direct link to DeveloperToolsOptions.Runner")
...
## DeveloperToolsOptions.Protocol[​](#developertoolsoptionsprotocol "Direct link to DeveloperToolsOptions.Protocol")
...
2. `DeveloperToolsProtocol.CreateHttp(Uri, TimeSpan)` - creates HTTP connection with provided parameters. Note: you need to reconfigure `Developer Tools` listener port independently by following [Settings page](/tools/developer-tools/settings.md).
...
## DeveloperToolsOptions.DiagnosticLogger[​](#developertoolsoptionsdiagnosticlogger "Direct link to DeveloperToolsOptions.DiagnosticLogger")

Defines sink to which all `AvaloniaUI.DiagnosticsSupport` logs are written to. By default, this option is set to `AvaloniaDiagnosticLogger`, redirecting logs to `Avalonia.Logger.TryGet`.
...
To learn more about `Developer Tools` logging, please read [Reporting Issues](/troubleshooting/tools/developer-tools.md).

## DeveloperToolsOptions.LoggerCollector[​](#developertoolsoptionsloggercollector "Direct link to DeveloperToolsOptions.LoggerCollector")

Defines a collector which listens for logs to be displayed in `Developer Tools`.

By default, `Developer Tools` will listen only to Avalonia logs and display them in the [Logger tools](/tools/developer-tools/logs-tool.md).
