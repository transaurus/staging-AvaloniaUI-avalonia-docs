...
## When to enable Win32 API shims[​](#when-to-enable-win32-api-shims "Direct link to When to enable Win32 API shims")
...
## Enabling Win32 API shims[​](#enabling-win32-api-shims "Direct link to Enabling Win32 API shims")
...
## Resolving `DllImportResolver` conflicts[​](#resolving-dllimportresolver-conflicts "Direct link to resolving-dllimportresolver-conflicts")
...
## Avoiding deadlocks with custom assembly loading[​](#avoiding-deadlocks-with-custom-assembly-loading "Direct link to Avoiding deadlocks with custom assembly loading")
...
See [Customizing Initialization](/xpf/configuration/customizing-initialization.md#custom-assembly-loading) for more details.

## Scope of Win32 API shims[​](#scope-of-win32-api-shims "Direct link to Scope of Win32 API shims")
...
* Not all Win32 APIs are available (see the [API reference](/xpf/third-party/winapi-reference.md))
...
### Application crashes on macOS or Linux with DllNotFoundException[​](#application-crashes-on-macos-or-linux-with-dllnotfoundexception "Direct link to Application crashes on macOS or Linux with DllNotFoundException")
...
### Application freezes during startup with AutoEnable[​](#application-freezes-during-startup-with-autoenable "Direct link to Application freezes during startup with AutoEnable")

If your application uses a custom assembly loading mechanism (such as a plugin system), `AutoEnable` can deadlock by intercepting assembly loads during startup. Switch to `AddLibrary` to register assemblies individually. See [Customizing Initialization: Custom Assembly Loading](/xpf/configuration/customizing-initialization.md#custom-assembly-loading).

### Shims are enabled but a specific library still fails[​](#shims-are-enabled-but-a-specific-library-still-fails "Direct link to Shims are enabled but a specific library still fails")

Some libraries call Win32 APIs that are not included in the shim layer. Check the [WinAPI Shim API Reference](/xpf/third-party/winapi-reference.md) to verify that the APIs your library needs are available. If a required API is missing, contact the Avalonia support team.

### EntryPointNotFoundException on Linux[​](#entrypointnotfoundexception-on-linux "Direct link to EntryPointNotFoundException on Linux")

If enabling Win32 API shims causes `EntryPointNotFoundException` for native Linux APIs (such as X11 functions), the shim layer is intercepting calls that should go to native libraries. Move native Linux API calls to a separate assembly and exclude it from the shim. See [Linux: Win32 API Shim Conflicts](/xpf/platforms/linux.md#win32-api-shim-conflicts-with-native-apis).

### A resolver is already set for the assembly[​](#a-resolver-is-already-set-for-the-assembly "Direct link to A resolver is already set for the assembly")
