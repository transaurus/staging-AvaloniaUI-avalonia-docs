...
## Reducing startup time with ReadyToRun[​](#reducing-startup-time-with-readytorun "Direct link to Reducing startup time with ReadyToRun")
...
On Linux, ReadyToRun may change how native `.so` libraries are resolved. See [Linux: Native Library Resolution](/xpf/platforms/linux.md#native-library-resolution-with-readytorun) for details.

## Rendering performance[​](#rendering-performance "Direct link to Rendering performance")

### Configuring Skia and composition options[​](#configuring-skia-and-composition-options "Direct link to Configuring Skia and composition options")

XPF uses Skia as its rendering engine. You can tune rendering performance through `SkiaOptions` and `CompositionOptions` in a [custom initialization](/xpf/configuration/customizing-initialization.md):
...
## Dynamic XAML loading[​](#dynamic-xaml-loading "Direct link to Dynamic XAML loading")
...
## Embedding high-performance content[​](#embedding-high-performance-content "Direct link to Embedding high-performance content")

For performance-critical rendering (such as real-time meters, audio visualizations, or 3D content), consider embedding Avalonia controls in your XPF application using [AvaloniaHost](/xpf/interop/embedding-avalonia-in-xpf.md). Avalonia's `CompositionCustomVisuals` API allows rendering directly on the composition thread, bypassing the WPF dispatcher entirely.
