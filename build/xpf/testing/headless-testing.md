...
Instead of automation testing, try Headless testing first as it's fast and portable. Since XPF is based on the same core as Avalonia, headless testing is available for WPF apps too.
...
For more detailed documentation on the Headless platform and Avalonia extensions, see [Headless Testing with XUnit](/docs/testing/headless-xunit.md) and [Headless Testing with NUnit](/docs/testing/headless-nunit.md). Understanding how headless testing works with Avalonia also helps with XPF/WPF.

## Configuring the testing project[​](#configuring-the-testing-project "Direct link to Configuring the testing project")

`XUnit`, `NUnit`, and `MSTest` are supported by XPF/Avalonia headless testing. It's necessary to include integration nuget package in the testing project:
...
## (Optional) Configuring the testing application[​](#optional-configuring-the-testing-application "Direct link to (Optional) Configuring the testing application")

Similarly to Avalonia headless, you can configure cross-platform `AppBuilder` to be used in the project. When not defined, headless platform is using default parameters, which might limit your XPF testing experience. Note, if you already override AppBuilder for your XPF app (as per [Customizing Initialization](/xpf/configuration/customizing-initialization.md) documentation), you can reuse the same initialization code, but add `.UseHeadless()` in the end of the chain.
...
Your tests are running in the same process as the XPF application, making it easier to send any events and access any output of the app. As with normal WPF app, everything has to start with a Window, which can be create in the same test method, or reused from set-up methods (`[SetUp]` method in NUnit or constructor in XUnit).

note

NUnit \[Test] and \[Theory] needs to be replaced with \[AvaloniaTest] \[AvaloniaTheory], as well as XUnit \[Fact] replaced with \[AvaloniaFact].
...
## Accessing Avalonia headless extensions[​](#accessing-avalonia-headless-extensions "Direct link to Accessing Avalonia headless extensions")
...
This extensions are only available on Avalonia Window, and can't be used on WPF Window. But lucky, it's possible to get Avalonia Window in headless tests:
...
See [Avalonia Interop](/xpf/interop/embedding-avalonia-in-xpf.md#accessing-avalonia-features) for more details on integration with Avalonia.

## (Optional) Using XPF headless testing with WPF app/project[​](#optional-using-xpf-headless-testing-with-wpf-appproject "Direct link to (Optional) Using XPF headless testing with WPF app/project")

Only startup project has to use Xpf.Sdk testing. Which also means that you can have normal "net8.0-windows" project with your controls, and reference them in XPF headless project.
...
## MSTest support[​](#mstest-support "Direct link to MSTest support")
...
## Running tests in CI[​](#running-tests-in-ci "Direct link to Running tests in CI")

When running XPF headless tests in CI environments on Linux:

* Ensure the license key is configured in the test project (see [Getting Started](/xpf/getting-started.md#step-4-add-your-licence-key))
...
* [Headless testing with XUnit](/docs/testing/headless-xunit.md)
* [Headless testing with NUnit](/docs/testing/headless-nunit.md)
* [Setting up the headless platform](/docs/testing/setting-up-the-headless-platform.md)
