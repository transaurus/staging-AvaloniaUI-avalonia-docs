...
### Using a custom Avalonia application[​](#using-a-custom-avalonia-application "Direct link to Using a custom Avalonia application")

Follow the steps in [Customizing Initialization](/xpf/configuration/customizing-initialization.md#optional-define-a-custom-avalonia-application) to create a custom Avalonia Application class, then set the `Name` property in your AXAML:
...
### Using DefaultXpfAvaloniaApplication[​](#using-defaultxpfavaloniaapplication "Direct link to Using DefaultXpfAvaloniaApplication")
...
## Native menus[​](#native-menus "Direct link to Native menus")

macOS applications use a global menu bar at the top of the screen. XPF supports this through Avalonia's `NativeMenu` API.

### Setting up a native menu programmatically[​](#setting-up-a-native-menu-programmatically "Direct link to Setting up a native menu programmatically")
...
### Cross-platform menu fallback[​](#cross-platform-menu-fallback "Direct link to Cross-platform menu fallback")
...
See [Embedding Avalonia in XPF](/xpf/interop/embedding-avalonia-in-xpf.md) for details on hosting Avalonia controls.

## Dock visibility[​](#dock-visibility "Direct link to Dock visibility")

To control whether your application appears in the macOS Dock, use `MacOSPlatformOptions` in a [custom initialization](/xpf/configuration/customizing-initialization.md):
...
### Info.plist interaction[​](#infoplist-interaction "Direct link to Info.plist interaction")
...
## Startup and modal dialogs[​](#startup-and-modal-dialogs "Direct link to Startup and modal dialogs")
...
### Recommended solutions[​](#recommended-solutions "Direct link to Recommended solutions")
...
## DPI and render scaling[​](#dpi-and-render-scaling "Direct link to DPI and render scaling")
...
## Trackpad gestures[​](#trackpad-gestures "Direct link to Trackpad gestures")
...
## GDI+ and System.Drawing.Common[​](#gdi-and-systemdrawingcommon "Direct link to GDI+ and System.Drawing.Common")
...
See [Library Compatibility](/xpf/third-party/compatibility.md) for more details.

## Packaging and deployment[​](#packaging-and-deployment "Direct link to Packaging and deployment")
...
## Key mapping[​](#key-mapping "Direct link to Key mapping")
...
### Automatic macOS key mapping[​](#automatic-macos-key-mapping "Direct link to Automatic macOS key mapping")

To fix many of these problems, one can call the `XpfKeyboard.MapMacOSKeys()` method on startup. This would usually be done in the same place as [the XPF WinAPI shim setup](/xpf/third-party/win32-api-shims.md); that is, in the constructor of your `App` class or `Program.Main`:
...
### macOS custom keyboard mapping[​](#macos-custom-keyboard-mapping "Direct link to macOS custom keyboard mapping")

For more flexible key mapping you can [add custom key mappings](/xpf/migration/key-mapping.md).

## Context menus[​](#context-menus "Direct link to Context menus")

On macOS, context menus can be opened by Ctrl+Clicking as well as by right clicking. You can enable this feature by setting `XpfMouse.ShowContextMenuOnMacOSCtrlClick` on startup. This would usually be done in the same place as [the XPF WinAPI shim setup](/xpf/third-party/win32-api-shims.md); that is, in the constructor of your `App` class or `Program.Main`:
...
## Native API interop[​](#native-api-interop "Direct link to Native API interop")
