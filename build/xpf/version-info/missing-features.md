...
## Features with limitations[​](#features-with-limitations "Direct link to Features with limitations")

The following features are available but have known restrictions:

* **`WebBrowser`**: XPF provides web content embedding through a different mechanism. See the [WebView docs](/docs/app-development/embedding-web-content.md) for details.
...
## Features planned for future releases[​](#features-planned-for-future-releases "Direct link to Features planned for future releases")
...
If your application depends on any of these features, check the [release notes](/xpf/version-info/release-notes.md) regularly for updates on their progress.

## Features unlikely to be supported[​](#features-unlikely-to-be-supported "Direct link to Features unlikely to be supported")

The following features are unlikely to be supported due to platform restrictions:

* **Multiple UI threads** (multiple `Dispatcher` instances): macOS allows only one UI thread, and Windows and Linux have limited support. See [Known differences from WPF](/xpf/migration/known-differences.md#multiple-ui-threads) for more detail.
...
* [Known differences from WPF](/xpf/migration/known-differences.md)
