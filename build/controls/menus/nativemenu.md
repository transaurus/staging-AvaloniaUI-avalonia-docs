...
The `NativeMenu` can display a menu on *macOS* and some Linux distributions. It can be used in several contexts:
...
| `Gesture`    | The keyboard shortcut associated with the menu item.                                               |
| `ToggleType` | The toggle behavior: `None` (default), `CheckBox`, or `Radio`. Uses the `MenuItemToggleType` enum. |
| `IsChecked`  | Whether the menu item is checked. Only applies when `ToggleType` is `CheckBox` or `Radio`.         |
...
### Gesture format[​](#gesture-format "Direct link to Gesture format")
...
* [NativeMenu API reference](/api/avalonia/controls/nativemenu.md)
* [`NativeMenu.cs` source code on GitHub](https://github.com/AvaloniaUI/Avalonia/blob/master/src/Avalonia.Controls/NativeMenu.cs)
* [macOS platform guide](/docs/platform-specific-guides/macos.md#dock-menu)
