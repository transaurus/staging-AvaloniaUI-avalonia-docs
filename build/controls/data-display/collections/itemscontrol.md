...
The [`ItemsControl`](/api/avalonia/controls/itemscontrol.md) is the base class for controls that display repeating data (for example, [`ListBox`](/api/avalonia/controls/listbox.md) and `ComboBox`). It has no built-in formatting, selection, or scroll behavior. You can use it with data binding, styling, and data templates to create a completely custom repeating data control.

tip

If you need built-in selection support, use [`ListBox`](/controls/data-display/collections/listbox.md) instead. If you need scrolling, wrap your `ItemsControl` in a `ScrollViewer`, or consider `ListBox` which includes one by default.
...
| `ItemsPanel`           | The panel that hosts generated items. Defaults to a `StackPanel`. See [custom ItemsPanel](/docs/custom-controls/custom-itemspanel.md) for details on replacing it. |
...
* **`ItemsControl` does not virtualize** its children. If you are working with a large number of items, consider using [`ItemsRepeater`](/controls/data-display/collections/itemsrepeater.md) with a virtualizing layout, or a `ListBox` which virtualizes by default.
...
![ItemsControl displaying a formatted list of crockery items](/assets/images/itemscontrol-with-custom-layout-and-formatting-8f1780eab3ce56a01e50893704fbb1ce.gif)
...
* [ItemsRepeater](/controls/data-display/collections/itemsrepeater.md)
* [Carousel](/controls/data-display/collections/carousel.md)
* [DataGrid](/controls/data-display/structured-data/datagrid.md)
* [Custom ItemsPanel](/docs/custom-controls/custom-itemspanel.md)
* [Data templates](/docs/data-binding/data-templates)
* [ItemsControl API reference](/api/avalonia/controls/itemscontrol.md)
