...
[`TabStrip`](/api/avalonia/controls/primitives/tabstrip.md) is composed of [`TabStripItem`](/api/avalonia/controls/primitives/tabstripitem.md) children. `TabStripItem`s are displayed in order of appearance and can be created by either: 1. a collection of `TabStripItem`s or 2. generated by `ItemsSource`.

Unlike [`TabControl`](/api/avalonia/controls/tabcontrol.md), `TabStrip` is not responsible for presenting the content of the selected tab. Instead, `TabStrip` requires you to respond to `SelectionChanged` events or `SelectedItem` property changes and then present the desired content with a separate control, such as `ContentControl`. This gives you full control over the displayed content and enables use cases such as custom view caching.

## Example: `TabStrip` with `TabStripItem`[​](#example-tabstrip-with-tabstripitem "Direct link to example-tabstrip-with-tabstripitem")
...
## Example: `TabStrip` with `ItemsSource`[​](#example-tabstrip-with-itemssource "Direct link to example-tabstrip-with-itemssource")
...
## When to use `TabStrip` vs `TabControl`[​](#when-to-use-tabstrip-vs-tabcontrol "Direct link to when-to-use-tabstrip-vs-tabcontrol")
...
* [TabStrip API reference](/api/avalonia/controls/primitives/tabstrip.md)
