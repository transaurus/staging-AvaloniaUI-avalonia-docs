...
[Parcel](/tools/parcel/packaging-for-linux.md) automates Linux packaging for Avalonia apps. It generates `.deb`, `.rpm`, and `.zip` packages with desktop integration handled for you. See the [Parcel setup guide](/tools/parcel/setup.md) to get started.

## Manual packaging[​](#manual-packaging "Direct link to Manual packaging")

The rest of this page covers how to create a `.deb` package manually using `dpkg-deb`.

In this tutorial, we will use the `dpkg-deb` tool to compile your `.deb` package.

### 1) Organize program files in a staging folder[​](#1-organize-program-files-in-a-staging-folder "Direct link to 1) Organize program files in a staging folder")
...
controlpackage control file

usr

bin

myprogramstarter script

lib

myprogram

libHarfBuzzSharp.soAvalonia native library

libSkiaSharp.soAvalonia native library

other\_native\_library\_1.so

myprogram\_executablemain executable
...
MyProgram.desktopdesktop shortcut file

icons

hicolor

...resolution icons (optional)

pixmaps

myprogram.pngmain application icon
...
### 2) Make the `control` file[​](#2-make-the-control-file "Direct link to 2-make-the-control-file")
...
### 3) Make the starter script[​](#3-make-the-starter-script "Direct link to 3) Make the starter script")
...
The **myprogram\_executable** file usually has the same name as its .NET project, e.g., if your Avalonia .csproj project is named *MyProgram.Desktop*, then the main executable generated by dotnet publish will be `MyProgram.Desktop`.
...
### 4) Make the desktop shortcut[​](#4-make-the-desktop-shortcut "Direct link to 4) Make the desktop shortcut")
...
### 5) Add hicolor icons (optional)[​](#5-add-hicolor-icons-optional "Direct link to 5) Add hicolor icons (optional)")
...
### 6) Compile the `.deb` package[​](#6-compile-the-deb-package "Direct link to 6-compile-the-deb-package")
...
## Example of a Linux shell script for the entire process[​](#example-of-a-linux-shell-script-for-the-entire-process "Direct link to Example of a Linux shell script for the entire process")
...
## To install[​](#to-install "Direct link to To install")
...
## To uninstall / remove[​](#to-uninstall--remove "Direct link to To uninstall / remove")
...
* [Desktop Linux platform integration](/docs/platform-specific-guides/linux.md)
