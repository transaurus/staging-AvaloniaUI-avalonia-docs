...
## Key bindings[​](#key-bindings "Direct link to Key bindings")

You can attach one or more [`KeyBinding`](/api/avalonia/input/keybinding.md) elements to any control's `KeyBindings` collection. Each `KeyBinding` maps a `Gesture` (a key, optionally combined with modifiers) to a command on your view model.
...
The `Gesture` string is parsed as a `KeyGesture`. You can use modifier shortcuts such as `Ctrl`, `Shift`, `Alt`, and `Cmd`. For a full list of supported keys and modifiers, see the [Keyboard and hotkeys](/docs/input-interaction/keyboard-and-hotkeys.md) reference.
...
## Handling double-click[​](#handling-double-click "Direct link to Handling double-click")
...
## Adding a context menu[​](#adding-a-context-menu "Direct link to Adding a context menu")
...
![Demo showing keyboard and mouse shortcut interactions with a ListBox](/assets/images/binding-key-mouse-test-c8621a926af37f748eee7f587ae49158.gif)

## Platform-specific notes[​](#platform-specific-notes "Direct link to Platform-specific notes")
...
* [Keyboard and hotkeys](/docs/input-interaction/keyboard-and-hotkeys.md): key bindings and hotkey configuration.
* [Commanding](/docs/input-interaction/commanding.md): the `ICommand` interface and command binding.
* [Gestures](/docs/input-interaction/gestures.md): tap, double-tap, and multi-pointer gesture recognizers.
