...
Controls that implement `ICommandSource` have a `HotKey` property that you can set or bind to. When the user presses the hotkey, Avalonia executes the command [bound](/docs/input-interaction/adding-interactivity.md) to that control.
...
## Keys and modifiers[​](#keys-and-modifiers "Direct link to Keys and modifiers")

A hotkey must have one [`Key`](/api/avalonia/input/key.md) and zero or more [`KeyModifiers`](/api/avalonia/input/keymodifiers.md). When you set a hotkey in XAML using the `HotKey` property, the string is parsed as a [`KeyGesture`](/api/avalonia/input/keygesture.md). Avalonia uses `Enum.Parse` to parse the key and modifiers, but you can also use common synonyms such as `Ctrl` instead of `Control`, or `Win` instead of `Meta`.

### Gesture string format[​](#gesture-string-format "Direct link to Gesture string format")
...
## Assigning number keys to hotkeys[​](#assigning-number-keys-to-hotkeys "Direct link to Assigning number keys to hotkeys")

When you need to bind number keys, use `D0` through `D9` for the main keyboard row, or `NumPad0` through `NumPad9` for the numeric keypad. See the full [`Key`](/api/avalonia/input/key.md) enumeration for all available values.
...
Access-key syntax such as `Content="_1"` does not register a hotkey. Use the `HotKey` property or a [`KeyBinding`](/api/avalonia/input/keybinding.md) instead.
...
## Common modifier keys[​](#common-modifier-keys "Direct link to Common modifier keys")
...
## Common hotkey patterns[​](#common-hotkey-patterns "Direct link to Common hotkey patterns")
...
* [`HotKeyManager`](/api/avalonia/controls/hotkeymanager.md)
* [`KeyGesture`](/api/avalonia/input/keygesture.md)
* [`KeyModifiers`](/api/avalonia/input/keymodifiers.md)
* [`Key`](/api/avalonia/input/key.md)
...
* [Focus](/docs/input-interaction/focus.md): Focus management and keyboard navigation.
* [Commanding](/docs/input-interaction/commanding.md): `ICommand` interface and command binding.
* [Adding interactivity](/docs/input-interaction/adding-interactivity.md): Events and commands overview.
* [Mouse and keyboard shortcuts](/docs/input-interaction/mouse-and-keyboard-shortcuts.md): Additional keyboard and mouse gesture handling.
