...
## .NET support updated[​](#net-support-updated "Direct link to .NET support updated")
...
## Avalonia version 12[​](#avalonia-version-12 "Direct link to Avalonia version 12")

The major version of Avalonia is now 12, up from 11.

Upgrade all Avalonia package references to the latest 12 patch version, either in your favorite IDE or by editing the project files.<br /><!-- -->Choose the latest release from the official [GitHub Releases page](https://github.com/AvaloniaUI/Avalonia/releases).
...
## `Avalonia.Diagnostics` package removed[​](#avaloniadiagnostics-package-removed "Direct link to avaloniadiagnostics-package-removed")

The `Avalonia.Diagnostics` package has been removed. The Dev Tools included with [Avalonia Accelerate](https://avaloniaui.net/accelerate) should be used instead.

Remove the `Avalonia.Diagnostics` package from your projects and replace it with `AvaloniaUI.DiagnosticsSupport`.<br /><!-- -->To install the Accelerate Dev Tools, see the [Dev Tools documentation](https://docs.avaloniaui.net/accelerate/tools/dev-tools/getting-started).
...
## Binding class hierarchy changes[​](#binding-class-hierarchy-changes "Direct link to Binding class hierarchy changes")
...
The `IBinding` interface has been removed. Its replacement is the [`BindingBase`](/api/avalonia/data/bindingbase.md) class.

All types of bindings now inherit from `BindingBase`: [`ReflectionBinding`](/api/avalonia/data/reflectionbinding.md), [`CompiledBinding`](/api/avalonia/data/compiledbinding.md), `TemplateBinding`, and `IndexerBinding`. Adjust your usage accordingly; don't assume that a `BindingBase` instance represents only a "standard" binding.
...
## Compiled bindings are enabled by default[​](#compiled-bindings-are-enabled-by-default "Direct link to Compiled bindings are enabled by default")

`<AvaloniaUseCompiledBindingsByDefault>` is now `true` by default.<br /><!-- -->Any `Binding` usage in XAML code now maps to `CompiledBinding`.
...
## Binding plugins removed[​](#binding-plugins-removed "Direct link to Binding plugins removed")
...
## Configurable text shaper[​](#configurable-text-shaper "Direct link to Configurable text shaper")
...
## Improved touch/pen focus and selection behavior[​](#improved-touchpen-focus-and-selection-behavior "Direct link to Improved touch/pen focus and selection behavior")

Selection handling in [`SelectingItemsControl`](/api/avalonia/controls/primitives/selectingitemscontrol.md) (and derived controls such as `ListBox`, `ComboBox`, `TabControl`) and [`TreeView`](/api/avalonia/controls/treeview.md) has been unified to provide consistent, platform-appropriate behavior across input devices:
...
### Obsoleted APIs[​](#obsoleted-apis "Direct link to Obsoleted APIs")
...
### Customizing selection behavior[​](#customizing-selection-behavior "Direct link to Customizing selection behavior")
...
## [`TopLevel`](/api/avalonia/controls/toplevel.md) changes[​](#toplevel-changes "Direct link to toplevel-changes")
...
## Window decoration changes[​](#window-decoration-changes "Direct link to Window decoration changes")
...
* A new [`WindowDrawnDecorations`](/api/avalonia/controls/chrome/windowdrawndecorations.md) class is added. Its role is to provide all the window decorations through a single control.
...
* The `Window.ExtendClientAreaChromeHints` property consisted of several flags that were not always behaving as expected. This property has been removed, as the [`WindowDecorations`](/api/avalonia/controls/windowdecorations.md) property in conjunction with `ExtendClientAreaToDecorationsHint` should be used instead.
...
## Clipboard changes[​](#clipboard-changes "Direct link to Clipboard changes")

In a previous version, clipboard support was rewritten to use a new [`IAsyncDataTransfer`](/api/avalonia/input/iasyncdatatransfer.md) interface and related types. To avoid breaking compatibility, the old `IDataObject` interface was kept (and made obsolete) as a shim for the new system.
...
Read the official [clipboard documentation](/docs/services/clipboard.md) for details on using `IAsyncDataTransfer`.
...
## Multiple dispatchers support[​](#multiple-dispatchers-support "Direct link to Multiple dispatchers support")
...
## `Dispatcher.InvokeAsync` captures the execution context[​](#dispatcherinvokeasync-captures-the-execution-context "Direct link to dispatcherinvokeasync-captures-the-execution-context")
...
## `FuncMultiValueConverter` accepts an `IReadOnlyList`[​](#funcmultivalueconverter-accepts-an-ireadonlylist "Direct link to funcmultivalueconverter-accepts-an-ireadonlylist")
...
## Data validation enabled by default in custom controls[​](#data-validation-enabled-by-default-in-custom-controls "Direct link to Data validation enabled by default in custom controls")
...
## Render target and platform surface interfaces reworked[​](#render-target-and-platform-surface-interfaces-reworked "Direct link to Render target and platform surface interfaces reworked")
...
* `ILockedFramebuffer` now includes an [`AlphaFormat`](/api/avalonia/platform/alphaformat.md) property.
...
## Text formatting constructors modified[​](#text-formatting-constructors-modified "Direct link to Text formatting constructors modified")
...
## Access keys are triggered by symbol[​](#access-keys-are-triggered-by-symbol "Direct link to Access keys are triggered by symbol")
...
## Font support updated[​](#font-support-updated "Direct link to Font support updated")
...
## [`Screen`](/api/avalonia/platform/screen.md) is abstract[​](#screen-is-abstract "Direct link to screen-is-abstract")
...
## [`ResourcesChangedEventArgs`](/api/avalonia/controls/resourceschangedeventargs.md) is a struct[​](#resourceschangedeventargs-is-a-struct "Direct link to resourceschangedeventargs-is-a-struct")
...
## Gesture events moved[​](#gesture-events-moved "Direct link to Gesture events moved")
...
### Direct2D1 support removed[​](#direct2d1-support-removed "Direct link to Direct2D1 support removed")
...
### `BinaryFormatter` removed[​](#binaryformatter-removed "Direct link to binaryformatter-removed")
...
### `Window.ExtendClientAreaToDecorationsHint` improved[​](#windowextendclientareatodecorationshint-improved "Direct link to windowextendclientareatodecorationshint-improved")
...
### App initialization changed[​](#app-initialization-changed "Direct link to App initialization changed")
...
1. Change your main activity to inherit from [`AvaloniaMainActivity`](/api/avalonia/android/avaloniamainactivity.md) (non-generic) instead of `AvaloniaMainActivity<TApp>` (generic).
...
### [`IActivityApplicationLifetime`](/api/avalonia/controls/applicationlifetimes/iactivityapplicationlifetime.md) replaces [`ISingleViewApplicationLifetime`](/api/avalonia/controls/applicationlifetimes/isingleviewapplicationlifetime.md)[​](#iactivityapplicationlifetime-replaces-isingleviewapplicationlifetime "Direct link to iactivityapplicationlifetime-replaces-isingleviewapplicationlifetime")
...
### Removed `CreateAppBuilder` and `CustomizeAppBuilder` from `AvaloniaMainActivity`[​](#removed-createappbuilder-and-customizeappbuilder-from-avaloniamainactivity "Direct link to removed-createappbuilder-and-customizeappbuilder-from-avaloniamainactivity")
...
### App initialization changed[​](#app-initialization-changed-1 "Direct link to App initialization changed")
...
### `Avalonia.Browser.Blazor` package removed[​](#avaloniabrowserblazor-package-removed "Direct link to avaloniabrowserblazor-package-removed")
...
## Tizen[​](#tizen "Direct link to Tizen")

### `Avalonia.Tizen` package removed[​](#avaloniatizen-package-removed "Direct link to avaloniatizen-package-removed")

The Tizen platform is no longer supported out of the box because it lacked a maintainer. Read [Moving Tizen Support Out of Main Repository](https://github.com/AvaloniaUI/Avalonia/discussions/19721) for details.

PR: [#19722](https://github.com/AvaloniaUI/Avalonia/pull/19722)

## Headless[​](#headless "Direct link to Headless")

### xUnit.net and NUnit supported versions updated[​](#xunitnet-and-nunit-supported-versions-updated "Direct link to xUnit.net and NUnit supported versions updated")
...
## Removed members[​](#removed-members "Direct link to Removed members")

The following members have been removed in Avalonia 12. They are grouped by the feature area that motivated the removal.

### Binding class hierarchy[​](#binding-class-hierarchy "Direct link to Binding class hierarchy")
...
### Binding plugins[​](#binding-plugins "Direct link to Binding plugins")
...
### Clipboard and drag-drop[​](#clipboard-and-drag-drop "Direct link to Clipboard and drag-drop")
...
### TopLevel[​](#toplevel "Direct link to TopLevel")
...
### Window decorations[​](#window-decorations "Direct link to Window decorations")
...
| `IPopupHostProvider` interface                         | [`Popup`](/api/avalonia/controls/primitives/popup.md) | [#20732](https://github.com/AvaloniaUI/Avalonia/pull/20732) |
...
### Gesture events[​](#gesture-events "Direct link to Gesture events")
...
### Other specific removals[​](#other-specific-removals "Direct link to Other specific removals")
...
### Obsolete since Avalonia 11[​](#obsolete-since-avalonia-11 "Direct link to Obsolete since Avalonia 11")
...
## Renamed members[​](#renamed-members "Direct link to Renamed members")
...
| `RenderOptions.TextRenderingMode`                         | `TextOptions.TextRenderingMode`       | `TextOptions` also includes `TextHintingMode` and `BaselinePixelAlignment`. See [Text options](/docs/graphics-animation/text-options.md).                                                                           | [#20107](https://github.com/AvaloniaUI/Avalonia/pull/20107) |
