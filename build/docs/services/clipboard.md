...
### Universal formats[​](#universal-formats "Direct link to Universal formats")

Universal formats are cross-platform formats that Avalonia directly understands.<br /><!-- -->There are currently three universal formats:

| Format              | Identifier | Type                                                             | Description         |
| ------------------- | ---------- | ---------------------------------------------------------------- | ------------------- |
| `DataFormat.Text`   | "Text"     | `string`                                                         | Plain text data     |
| `DataFormat.File`   | "File"     | [`IStorageItem`](/api/avalonia/platform/storage/istorageitem.md) | A file or directory |
| `DataFormat.Bitmap` | "Bitmap"   | [`Bitmap`](/api/avalonia/media/imaging/bitmap.md)                | A bitmap image      |

### Platform formats[​](#platform-formats "Direct link to Platform formats")
...
Do **not** assume that a given identifier works for all platforms!<br /><!-- -->For example, the HTML format is named `HTML format` on Windows, `text/html` on Linux and Android, and `public.html` on macOS and iOS. Always verify the intended operating system before using a platform-specific format.
...
### Application formats[​](#application-formats "Direct link to Application formats")
...
## IClipboard[​](#iclipboard "Direct link to IClipboard")
...
An instance of `IClipboard` can be accessed through a [`TopLevel`](/docs/fundamentals/top-level.md) object:
...
#### TryGetDataAsync()[​](#trygetdataasync "Direct link to TryGetDataAsync()")

The clipboard's content is read by retrieving an instance of [`IAsyncDataTransfer`](/api/avalonia/input/iasyncdatatransfer.md). This object is responsible for providing data items of various formats on demand (see the [`IAsyncDataTransfer`](#iasyncdatatransfer--iasyncdatatransferitem) section below).
...
#### TryGetInProcessDataAsync()[​](#trygetinprocessdataasync "Direct link to TryGetInProcessDataAsync()")
...
### Writing[​](#writing "Direct link to Writing")

#### SetDataAsync()[​](#setdataasync "Direct link to SetDataAsync()")
...
#### Extension methods[​](#extension-methods-1 "Direct link to Extension methods")
...
#### ClearAsync()[​](#clearasync "Direct link to ClearAsync()")

Calling the `ClearAsync()` method clears the clipboard of all contents.

#### FlushAsync()[​](#flushasync "Direct link to FlushAsync()")

On Windows, macOS, and X11, the data is retrieved lazily on demand from the `IAsyncDataTransfer` placed on the clipboard. If the Avalonia application terminates without flushing, the data becomes unavailable.
...
## IAsyncDataTransfer & IAsyncDataTransferItem[​](#iasyncdatatransfer--iasyncdatatransferitem "Direct link to IAsyncDataTransfer & IAsyncDataTransferItem")
...
* `Items`, returning a list of [`IAsyncDataTransferItem`](/api/avalonia/input/iasyncdatatransferitem.md) instances representing all items present inside the object.
...
A single item can have multiple formats.<br /><!-- -->For example, rich text might be represented in RTF, HTML, and plain text.

### Getting values[​](#getting-values "Direct link to Getting values")
...
The returned value is untyped (`object`).<br /><!-- -->Consider using the extension methods described below to get typed results.

#### Typed[​](#typed "Direct link to Typed")
...
#### DataTransfer & DataTransferItem[​](#datatransfer--datatransferitem "Direct link to DataTransfer & DataTransferItem")

To provide values to write to the clipboard, both `IAsyncDataTransfer` and `IAsyncDataTransferItem` must be implemented.

Avalonia provides implementations of those interfaces with the `DataTransfer` and [`DataTransferItem`](/api/avalonia/input/datatransferitem.md) types, respectively:
...
* [Drag and Drop](/docs/input-interaction/drag-and-drop.md): Drag-and-drop data transfer using similar data format APIs.
