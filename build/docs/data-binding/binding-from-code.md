...
## Creating compiled bindings from code[​](#creating-compiled-bindings-from-code "Direct link to Creating compiled bindings from code")

The [`CompiledBinding.Create`](/api/avalonia/data/compiledbinding.md#create-method) method lets you create type-safe bindings using LINQ expressions. The expression is validated at compile time, so errors in property names produce compiler errors rather than silent runtime failures.

The method takes two generic parameters: the type of the [`DataContext`](/docs/data-binding/data-context.md) and the type of the property being selected. It then accepts a lambda expression to select the property to bind.

For example, if you have a control whose [`DataContext`](/docs/data-binding/data-context.md) is an instance of `MyViewModel`, and you want to select a `string Title { get; set; }` property from it, you would write:
...
The [`CompiledBinding.Create`](/api/avalonia/data/compiledbinding.md#create-method) method takes various optional parameters to control the binding.
...
This approach gives you the same performance and safety benefits as XAML compiled bindings, but from C# code. See [Compiled Bindings](/docs/data-binding/compiled-bindings.md) for the XAML equivalent.

## Subscribing to property changes[​](#subscribing-to-property-changes "Direct link to Subscribing to property changes")

You can subscribe to [`AvaloniaObject`](/api/avalonia/avaloniaobject.md) property changes by calling the [`GetObservable`](/api/avalonia/avaloniaobjectextensions.md#getobservable-method) extension method. This returns an [`IObservable<T>`](https://learn.microsoft.com/en-us/dotnet/api/system.iobservable-1?view=net-10.0) which can be used to listen for changes to the property:
...
Alternatively, you can subscribe to the [`AvaloniaObject.PropertyChanged`](/api/avalonia/avaloniaobject.md#propertychanged-event) event, which fires whenever *any* property changes on the element.
...
## Binding to an observable[​](#binding-to-an-observable "Direct link to Binding to an observable")

You can bind a property to an observable using the [`AvaloniaObject.Bind`](/api/avalonia/avaloniaobject.md#bind-method-1) method:
...
## Setting a binding in an object initializer[​](#setting-a-binding-in-an-object-initializer "Direct link to Setting a binding in an object initializer")
...
## Using reflection bindings from code[​](#using-reflection-bindings-from-code "Direct link to Using reflection bindings from code")
...
## Subscribing to a property on any object[​](#subscribing-to-a-property-on-any-object "Direct link to Subscribing to a property on any object")
...
To do this you can subscribe to [`AvaloniaProperty.Changed`](/api/avalonia/avaloniaproperty.md) which is an observable which fires *every time the property is changed on any instance*.
...
## Inspecting active bindings[​](#inspecting-active-bindings "Direct link to Inspecting active bindings")
...
* [Data Binding Syntax](/docs/data-binding/data-binding-syntax.md): XAML binding syntax reference.
* [Compiled Bindings](/docs/data-binding/compiled-bindings.md): Compile-time validated bindings.
* [Binding Debugging](/docs/data-binding/binding-debugging.md): Diagnosing binding issues.
