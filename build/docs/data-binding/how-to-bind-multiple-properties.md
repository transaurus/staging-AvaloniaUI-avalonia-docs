...
When a single target property depends on values from several sources, you can use [`MultiBinding`](/api/avalonia/data/multibinding.md) to aggregate multiple `Binding` objects and produce a combined result through an [`IMultiValueConverter`](/api/avalonia/data/converters/imultivalueconverter.md). The converter's `Convert` method runs each time any of the bound properties raises a change notification, so the target property stays in sync automatically.
...
* [Data binding syntax](/docs/data-binding/data-binding-syntax.md) and how `Binding` expressions work.
* Declaring resources in XAML with `x:Key` so you can reference converters via `StaticResource`.

## Understand `IMultiValueConverter`[​](#understand-imultivalueconverter "Direct link to understand-imultivalueconverter")
...
## Bind RGB sliders to a foreground brush[​](#bind-rgb-sliders-to-a-foreground-brush "Direct link to Bind RGB sliders to a foreground brush")
...
### Step 1: Define the XAML layout[​](#step-1-define-the-xaml-layout "Direct link to Step 1: Define the XAML layout")
...
### Step 2: Implement the converter[​](#step-2-implement-the-converter "Direct link to Step 2: Implement the converter")
...
![App showing RGB sliders bound to multiple properties producing a combined color](/assets/images/multibinding-rgb-e99dc0f20485f69304c38c9edc8313dc.gif)

## Simplify with `FuncMultiValueConverter`[​](#simplify-with-funcmultivalueconverter "Direct link to simplify-with-funcmultivalueconverter")
...
* [MultiBinding](/docs/data-binding/multi-binding.md): Full `MultiBinding` reference, including `StringFormat`, `FallbackValue`, and the properties table.
* [How to create a custom data binding converter](/docs/data-binding/how-to-create-a-custom-data-binding-converter.md): Single-value `IValueConverter` implementations.
* [Built-in data binding converters](/docs/data-binding/built-in-data-binding-converters.md): Converters shipped with Avalonia.
* [Data binding syntax](/docs/data-binding/data-binding-syntax.md): Binding parameters including `StringFormat` and `ConverterParameter`.
