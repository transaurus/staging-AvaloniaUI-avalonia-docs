# Embedding Android native views

Avalonia lets you embed native Android views inside the Avalonia visual tree by subclassing [`NativeControlHost`](/api/avalonia/controls/nativecontrolhost.md). You wrap each Android `View` in an [`AndroidViewControlHandle`](/api/avalonia/android/androidviewcontrolhandle.md) and return it from `CreateNativeControlCore`. This is useful when you need platform-specific controls (such as `WebView`, `MapView`, or media players) that have no Avalonia equivalent.
...
## Getting the parent context[​](#getting-the-parent-context "Direct link to Getting the parent context")
...
## Example: embedding a WebView and a Button[​](#example-embedding-a-webview-and-a-button "Direct link to Example: embedding a WebView and a Button")
...
* [Developing with Avalonia for Android](/docs/platform-specific-guides/android.md)
