...
## UI icons[​](#ui-icons "Direct link to UI icons")
...
### Using image files[​](#using-image-files "Direct link to Using image files")
...
In this example, an [`Image`](/api/avalonia/controls/image.md) control is used to display an image from the application's resources as an icon. The `Source` property of the `Image` control is set to a resource URI that points to the image file.

### Using icon fonts[​](#using-icon-fonts "Direct link to Using icon fonts")
...
### Using path icons[​](#using-path-icons "Direct link to Using path icons")

Path icons can draw icons from `Geometry` which includes using paths from the scalable vector graphics (SVG) format that can be customized with size and color. See the [reference](/controls/media/pathicon.md) for how to use this control.
...
* [Shapes and Geometries](/docs/graphics-animation/shapes-and-geometries.md): Geometry types for path icons.
