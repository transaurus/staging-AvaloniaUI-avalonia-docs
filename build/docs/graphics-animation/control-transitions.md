# Setting control transitions

Transitions in Avalonia are also heavily inspired by CSS Animations. They listen to any changes in target property's value and subsequently animates the change according to its parameters. They can be defined on any `Control` via the [`Transitions`](/api/avalonia/animation/transitions.md) property:
...
The easing functions are the same as those described in [Keyframe Animations](/docs/graphics-animation/keyframe-animations.md#easing-function).
...
## Transitioning render transforms[​](#transitioning-render-transforms "Direct link to Transitioning render transforms")
