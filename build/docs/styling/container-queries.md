...
## How to use queries[​](#how-to-use-queries "Direct link to How to use queries")

### Declaring container queries[​](#declaring-container-queries "Direct link to Declaring container queries")
...
The `Name` property defines the name of the container it will attach to. This isn't a unique identifier, and multiple container queries can use the same name. The `Query` defines the rules to activate the containing size. See [Queries](#queries) below.
...
1. Container Queries can't be hosted in a `Style` element. The following is invalid.
...
### Declaring containers[​](#declaring-containers "Direct link to Declaring containers")
...
* [Responsive layouts](/docs/layout/responsive-layouts.md): Using container queries to build adaptive layouts.
