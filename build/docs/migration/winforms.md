...
| `Form`                                                 | `Window`                                          |                                                         |
| [`UserControl`](/api/avalonia/controls/usercontrol.md) | `UserControl`                                     | Same name, different base class                         |
...
### Option 1: start fresh, migrate incrementally[​](#option-1-start-fresh-migrate-incrementally "Direct link to Option 1: start fresh, migrate incrementally")
...
### Option 2: host Avalonia controls inside WinForms (Windows only)[​](#option-2-host-avalonia-controls-inside-winforms-windows-only "Direct link to Option 2: host Avalonia controls inside WinForms (Windows only)")
...
For setup instructions, see [Embedding Avalonia in Windows Forms](/docs/platform-specific-guides/windows.md#embedding-avalonia-in-windows-forms).

## Key concepts to learn[​](#key-concepts-to-learn "Direct link to Key concepts to learn")

If you are coming from WinForms with no XAML experience, these are the areas that will require the most adjustment:

* **[XAML basics](/docs/xaml.md):** Avalonia uses XAML to declare UI layout and structure. This replaces the WinForms designer.
...
* **[Styling](/docs/styling/styles.md):** Avalonia uses a CSS-like styling system with selectors and style classes, rather than setting properties on individual controls.
...
* [Embedding Avalonia in Windows Forms](/docs/platform-specific-guides/windows.md#embedding-avalonia-in-windows-forms): Host Avalonia controls inside an existing WinForms app.
